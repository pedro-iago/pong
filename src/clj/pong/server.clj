...
(defn pong-handler [conn]
  (wrap-resource
    (wrap-transit-response
      (wrap-transit-params (wrap-connection handler conn)))
    "public"))

(defn pong-handler-dev [conn]
  (fn [req]
    ((pong-handler conn) req)))
...
(defn dev-server [web-port]
  (WebServer. web-port pong-handler-dev true nil))

(defn prod-server []
  (WebServer. nil pong-handler false nil))

;; =============================================================================
;; Route Testing

(comment
  (require '[pong.core :as cc])
